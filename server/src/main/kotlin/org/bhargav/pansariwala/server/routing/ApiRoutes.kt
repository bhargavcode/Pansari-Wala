...
fun Route.apiRoutes(config: ServerConfig, store: AppStore) {
    ...
    authenticate("auth-jwt") {
        ...
        put("/me/profile") {
            ...
            call.respond(store.updateProfile(call.userId(), body.name, body.address, body.lat, body.lng))
        }
            ...
            runCatching { store.acceptOffer(call.userId(), call.parameters["id"]!!) }
                ...
                .onFailure {
                    if (it.message == "ALREADY_TAKEN") {
                        call.respond(HttpStatusCode.Conflict, store.offerById(call.parameters["id"]!!, call.userId()))
                    } else throw it
                }
        }
